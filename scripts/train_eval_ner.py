...
def load_and_cache_examples(cfg, tokenizer, labels, pad_token_label_id, mode):
    ...
    cached_features_file = os.path.join(
        cfg.data.path,
        "cached_{}_{}_{}".format(
            mode, list(filter(None, cfg.model.name_or_path.split("/"))).pop(), str(cfg.data.max_seq_length)
        ),
    )
...
def train(cfg, train_dataset, model, tokenizer, labels, pad_token_label_id, device):
    """ Train the model """
    if cfg.device.local_rank in [-1, 0]:
        tb_writer = SummaryWriter()

    cfg.data.train_batch_size = cfg.data.per_gpu_train_batch_size * max(1, cfg.device.n_gpu)
    train_sampler = RandomSampler(train_dataset) if cfg.device.local_rank == -1 else DistributedSampler(train_dataset)
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=cfg.data.train_batch_size)

    if cfg.optim.max_steps > 0:
        t_total = cfg.optim.max_steps
        cfg.optim.num_train_epochs = cfg.optim.max_steps // (len(train_dataloader) // cfg.optim.gradient_accumulation_steps) + 1
    else:
        t_total = len(train_dataloader) // cfg.optim.gradient_accumulation_steps * cfg.optim.num_train_epochs
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=cfg.optim.learning_rate, eps=cfg.optim.adam_epsilon)
    ...
    if os.path.isfile(os.path.join(cfg.model.name_or_path, "optimizer.pt")) and os.path.isfile(
        ...
        optimizer.load_state_dict(torch.load(os.path.join(cfg.model.name_or_path, "optimizer.pt")))
        scheduler.load_state_dict(torch.load(os.path.join(cfg.model.name_or_path, "scheduler.pt")))
    ...
    if cfg.device.local_rank != -1:
        model = torch.nn.parallel.DistributedDataParallel(
            model, device_ids=[cfg.device.local_rank], output_device=cfg.device.local_rank, find_unused_parameters=True
        )
    ...
    logger.info("  Instantaneous batch size per GPU = %d", cfg.data.per_gpu_train_batch_size)
    logger.info(
        "  Total train batch size (w. parallel, distributed & accumulation) = %d",
        cfg.data.train_batch_size
        * cfg.optim.gradient_accumulation_steps
        * (torch.distributed.get_world_size() if cfg.device.local_rank != -1 else 1),
    )
    logger.info("  Gradient Accumulation steps = %d", cfg.optim.gradient_accumulation_steps)
    ...
    if os.path.exists(cfg.model.name_or_path):
        ...
        try:
            global_step = int(cfg.model.name_or_path.split("-")[-1].split("/")[0])
        except ValueError:
            global_step = 0
        epochs_trained = global_step // (len(train_dataloader) // cfg.optim.gradient_accumulation_steps)
        steps_trained_in_current_epoch = global_step % (len(train_dataloader) // cfg.optim.gradient_accumulation_steps)
    ...
    for _ in train_iterator:
        epoch_iterator = tqdm(train_dataloader, desc="Iteration", position=0, leave=True, disable=cfg.device.local_rank not in [-1, 0])
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % cfg.optim.gradient_accumulation_steps == 0:
                ...
                if cfg.device.local_rank in [-1, 0] and cfg.logging.steps > 0 and global_step % cfg.logging.steps == 0:
                    ...
                    tb_writer.add_scalar("loss", (tr_loss - logging_loss) / cfg.logging.steps, global_step)
                    logging_loss = tr_loss

                if cfg.device.local_rank in [-1, 0] and cfg.model.save_steps > 0 and global_step % cfg.model.save_steps == 0:
                    # Save model checkpoint
                    output_dir = os.path.join(cfg.model.output_dir, "checkpoint-{}".format(global_step))
...
def start_training(cfg: DictConfig):
    ...
    if cfg.device.server_ip and cfg.device.server_port:
        ...
        ptvsd.enable_attach(address=(cfg.device.server_ip, cfg.device.server_port), redirect_output=True)
        ptvsd.wait_for_attach()

    # Setup CUDA, GPU & distributed training
    if cfg.device.local_rank == -1 or not cfg.device.no_cuda:
        device = torch.device("cuda" if torch.cuda.is_available() and not cfg.device.no_cuda else "cpu")
    ...
    logging.basicConfig(
        ...
        datefmt="%m/%d/%Y %H:%M:%S",
        level=logging.INFO if cfg.device.local_rank in [-1, 0] else logging.WARN,
    )
    ...
    config_class, model_class, tokenizer_class = AutoConfig, AutoModelForTokenClassification, AutoTokenizer #MODEL_CLASSES[cfg.model.type]
    ...
    if cfg.experiment.do.train:
        train_dataset = load_and_cache_examples(cfg, tokenizer, labels, pad_token_label_id, mode="train")
        #train_dataset = load_examples(cfg, mode="train")
        global_step, tr_loss = train(cfg, train_dataset, model, tokenizer, labels, pad_token_label_id, device)
    ...
    if cfg.experiment.do.finetune:
        tokenizer = tokenizer_class.from_pretrained(cfg.model.input_dir, do_lower_case=cfg.model.do_lower_case)
    ...
    if cfg.do_eval and cfg.device.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(device)
            result, _ = evaluate(cfg, model, tokenizer, labels, pad_token_label_id, mode="dev", device=device, prefix=global_step)
            if global_step:
                result = {"{}_{}".format(global_step, k): v for k, v in result.items()}
            results.update(result)
        output_eval_file = os.path.join(cfg.model.output_dir, "eval_results.txt")
    ...
    if cfg.do_predict and cfg.device.local_rank in [-1, 0]:
        ...
        # Save predictions
        output_test_predictions_file = os.path.join(cfg.model.output_dir, "test_predictions.txt")
