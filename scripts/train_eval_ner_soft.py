...
def load_and_cache_examples(cfg, tokenizer, labels, pad_token_label_id, mode):
    ...
    if os.path.exists(cached_features_file) and not cfg.model.overwrite_cache:
        ...
        features, vals = convert_examples_to_features(
            examples,
            labels,
    ...
    all_soft_votes = torch.tensor(
        [f.soft_votes for f in features], dtype=torch.float32)
...
def train(cfg, train_dataset, model, tokenizer, labels, pad_token_label_id, device):
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            logits = F.log_softmax(outputs.get("logits"), dim=-1)
            
            # loss
            loss = -torch.sum(logits * soft_votes, dim=-1).mean()
            ...
            if (step + 1) % cfg.optim.gradient_accumulation_steps == 0:
                ...
                if cfg.device.local_rank in [-1, 0] and cfg.logging.steps > 0 and global_step % cfg.logging.steps == 0:
                    ...
                    if (
                        ...
                        results, _ = evaluate(
                            cfg, model, tokenizer, labels, pad_token_label_id, mode="dev", device=device)
...
def start_training(cfg: DictConfig):
    ...
    if cfg.experiment.do.train:
        ...
        global_step, tr_loss = train(
            cfg, train_dataset, model, tokenizer, labels, pad_token_label_id, device)
    ...
    if cfg.experiment.do.finetune:
        ...
        result, predictions = evaluate(
            cfg, model, tokenizer, labels, pad_token_label_id, mode="test", device=device)
        train_dataset = load_and_cache_examples(
            cfg, tokenizer, labels, pad_token_label_id, mode="train")
    ...
    if cfg.experiment.do.eval and cfg.device.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            result, _ = evaluate(cfg, model, tokenizer, labels, pad_token_label_id,
                                 mode="dev", device=device, prefix=global_step)
